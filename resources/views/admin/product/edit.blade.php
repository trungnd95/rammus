@extends('admin.main')
@section('title', 'Khu vực quản trị của website Rammus | Edit Product')
@section ('content')
					...
					<div class="box-body">
						<div class="form-group">
		                  ...
		                  <select class="form-control" name="category">
		                    ...
		                    {!! $objCategory->create_select_category($parent_cate, 0, '', old('category', isset($product['cate_id']) ? $product['cate_id'] : 0)) !!}
		                  </select>
		                </div>
						<div class="form-group">
		                  <label for="name">Name <span>*</span></label>
		                  <input class="form-control" type="text" placeholder="Name" name="name" id="name" value="{{ old('name', isset($product['name']) ? $product['name'] : null) }}">
		                </div>
		                <div class="form-group">
		                  <label for="price">Price</label>
		                  <input class="form-control" type="text" placeholder="Price" name="price" id="price" value="{{ old('price', isset($product['price']) ? $product['price'] : null) }}">
		                </div>
		                <div class="form-group">
		                	<label for="Image">Image ID <span>*</span></label>
		                	<input type="text" class="image_id form-control" disabled name="disabled_img_id"  value="{{ old('disabled_img_id', isset($product['image']) ? $product['image'] : null) }}" />
		                	<input type="hidden" name="image" id="Image" class="form-control image_id"  value="{{ old('image', isset($product['image']) ? $product['image'] : null) }}"/>
		                </div>
		                <div class="form-group">
		                ...
		                <div class="form-group">
		                  ...
		                  <textarea class="form-control" rows="6" placeholder="Info ..." name="info" id="info">{{ old('info', isset($product['info']) ? $product['info'] : null) }}</textarea>
		                  <script>
	                  	ckeditor ("info")
					  </script>
	                </div>  
            ...
            <div class="box-body" style="display: block;">
              <div class="form-group">
              		<?php 
              			...
              			$array_color = unserialize($product['color']);
              		?>
	                      ...
	                      <div>
	                      	<input type="checkbox" name="colors[]" value="{{ $item->id }}" <?php 
		                      	if(!empty(old('colors'))) {
		                      		...
		                      		echo (is_numeric(array_search($item->id, $array_color)) == TRUE) ? 'checked="checked"' : '';
		                      	}
              ...
              <div class="form-group">
		      	...
		      	<?php 
              		...
              		$array_size = unserialize($product['size']);
              	?>
		      	<div class="row">
                  	<div class="col-lg-6">
	                      ...
	                      <div>
	                      	<input type="checkbox" name="sizes[]" value="{{ $item->id }}" <?php 
		                      	if(!empty(old('sizes'))) {
		                      		...
		                      		echo (is_numeric(array_search($item->id, $array_size)) == TRUE) ? 'checked="checked"' : '';
		                      	}
	                      	?> style="width: 22px;height: 22px;margin-top: 0px;">
	                      </div>
	                      <div style="margin-left: 5px;">
